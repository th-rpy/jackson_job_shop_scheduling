...
def create_pdf_file(pdf_name = 'output/Algo_Cds_Output.pdf'): 
    ...
    ptext = 'Example: n = 5 Tasks through m = 4 Machines. Above the duration table (pij) :'
    Story.append(Paragraph(ptext, styles["Justify"]))
    Story.append(Spacer(1, 20))
    data=[('Job / Machine', 'M 1', 'M 2', 'M 3', 'M4'), 
          ('J 1', '5', '4', '7', '2'), 
          ('J 2', '7', '8', '3', '4'),
          ('J 3', '6', '3', '4', '9'),
          ('J 4', '4', '6', '7', '6'),
...
def add_table_to_pdf(data, Story):
    table = Table(data, [0.9 * inch, 0.8 * inch, 0.8 * inch, 0.8 * inch, 0.8 * inch, 0.8 * inch],
                   (len(data))*[0.5 * inch])
...
al = JackAlgo([[1, 7, 5, 6,9,10], [2, 4, 6, 5, 8,1], 
                           [3, 8, 2, 4,3,7], 
                           [4, 6, 3, 9,7,5], [5, 5, 7, 3,5,9]])
preparedData = al.prepareData()
cmaxVirtual, _ , __ = al.get_cmax_virtual(preparedData)
al.solve(cmaxVirtual)
al.solve(cmaxVirtual) """